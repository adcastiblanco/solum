...
- Clinical history ("clinical.history") — PAST: prior treatments and medications already tried (and when they were started), prior responses and outcomes, relevant past medical / family / social history, prior assessment or test results referenced for comparison (e.g. "was <prior value> on <prior date>"), and the assessment/impression statement that links past to present. If the past information is embedded in a narrative Subjective section rather than a separate PMH header, EXTRACT THOSE PAST PHRASES anyway — write a concise sentence capturing only the past elements following this shape: "Started <medication/therapy> <dose/frequency> <duration> ago; prior <assessment or measurement> was <value> on <date>; <diagnosis or condition> <severity/stage>, <trajectory>". Adapt the slots to whatever specialty the document is from (behavioral health, cardiology, oncology, primary care, etc.) — only include slots the document supports. Do NOT return null just because the document lacks a dedicated history section. Do NOT include current symptoms in full (those belong in presenting_symptoms), and do NOT include the forward plan.
...
- Assessment scores ("clinical.assessments"): TODAY'S validated assessment scores or specialty-standard measurements from the current visit only. One row per instrument, with tool / score / date. The "tool" is whatever validated scale, scoring system, staging system, or quantitative measurement is appropriate for the specialty — behavioral health (PHQ-9, GAD-7, PCL-5, AUDIT), cardiology (NYHA class, ejection fraction), oncology (ECOG, Karnofsky, TNM stage), neurology (NIHSS, GCS, MMSE), pain (NRS, VAS), vitals (BP, HR, SpO2), and so on. If the document shows a current value alongside a historical one (e.g. "<tool>: <current value> (was <prior value> on <prior date>)"), the current row is { tool: "<tool>", score: "<current value>", date: "<today's visit date>" }. The prior value belongs in clinical.history, NOT here. Omit purely historical scores; this field is the latest measurement only.
